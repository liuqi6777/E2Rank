...
def test_pair_gradient_matches_cellwise_oracle_and_only_credits_endpoints(scale, chunk, cached, estimator):
    ...
    with torch.autocast('cpu', dtype=torch.bfloat16):
        function = (pairwise_shortlist_loss if estimator == 'conditional_projection'
                    else rloo_pairwise_shortlist_loss)
        loss, stats = function(means[:,0], means[:,1:], q, docs, positives,
...
@pytest.mark.parametrize('function', [pairwise_shortlist_loss, rloo_pairwise_shortlist_loss])
def test_ties_zero_pairs_and_collinear_directions_are_finite(no_pairs, function):
    ...
    loss, stats = function(h[:,0],h[:,1:],q,docs,
...
@pytest.mark.parametrize('estimator', ['conditional_projection', 'score_function'])
def test_two_rank_pair_gradient_with_uneven_tails_and_empty_cross_pool(tmp_path, estimator):
