...
更新日期：2026-09-17。输入协议修订：`64d2e7d`；条件投影实现基线：`54e9fbf`。

### 2026-09-17 结果呈现决策

已完成 G1-R2 核心、78 次消融、三次动态检索及 DIVER 核心实验，结果见 [结果文档](G1_R2_RESULTS.md)。**主结果采用 graded CP/G64/alignment 0.80，并与 SF-0.80 配对比较；原 0.90 作为探索消融点，已有机制消融与梯度探针继续使用 0.90 的匹配对照。** 0.80 是本轮探索消融后的选择，不追溯修改历史运行合同、run 名称或配置。DIVER 仅作内部记录，不纳入论文正文、附录、实验计数或主张。下文保留原批次计划与入口开发时的状态记录。
...
每个入口可用 `--variants`、`--seeds` 拆任务，默认核心队列不包含后两组。
Alignment 扩至 0.40/0.53/0.65/0.80/0.90/0.95/0.98，0.90 复用现有 SF/CP 三 seed；
退火为 0.80→0.90；Gaussian 为采样失配诊断，并配置同 κ 的 vMF 对照。
本节更新后续安排，不改下面已完成主批次的历史运行合同；下文“不自动追加”仍指原夜跑队列。

另已接入独立的 [R2 动态检索入口](../docs/g1_r2_dynamic.md)：固定 E0 文档全库索引，
query-only SF、G64、graded nDCG@10、三 seed；最终 BRIGHT 也冻结文档 encoder。
该路线不使用 CP，不作为 joint 静态配方的单因素消融；索引编码是新增前置成本。
