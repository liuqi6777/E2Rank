...
4. RAG 另用含答案的 QA train/test 数据，固定训练预算并只评测最终 checkpoint。训练 passage qrels 由 DPR NQ 人工 positive context 与 HotpotQA supporting facts 预先映射到冻结 corpus；答案包含只作诊断，不作训练标签。ReasonRank 排序标签不直接提供 downstream answer-F1 训练监督，也不能替代 RAG qrels。固定 generator 和索引，隔离 query adaptation 效果。
