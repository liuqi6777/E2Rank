...
0.6B 的补实验已汇入[独立 suite 与导入脚本](../docs/iclr2027_final_k0_suite.md)，输出根目录为 `checkpoints/iclr2027-final-k0/`；新训练仍待执行，旧结果只能在存有 checkpoint 的训练机上复制。4B 的 `K=0 + pairwise` 三 seed 补实验已加入[现有 4B 启动脚本](../docs/qwen3_embedding_4b_lora.md)，用 `--set k0` 单独启动，学习率为 `2e-4`；尚未训练。

配套的 LaTeX 表体初稿在本地 `iclr2027_table_drafts/proposed_*.tex`，已由 Git 忽略。论文现已在 `iclr2027/tables/` 中接入独立的新表体，未完成结果保持空格；`build_results.py` 继续生成旧配方的历史表，不覆盖新表。0.6B 数字取自 [`g1_r2_bright`](_summary/g1_r2_bright/run_summary.csv)，4B 数字取自 [`g1_r2_bright_4b`](_summary/g1_r2_bright_4b/run_summary.csv)及[4B calibration](_summary/g1_r2_bright_4b_calibration/run_summary.csv)，各领域使用对应目录的 `subset_summary.csv`。分数按 nDCG@10 × 100；消融表的 StackExchange、Coding、Theorem-based 分别是 7、2、3 个 subset 的等权均值，再对三个 seed 取均值；Avg. 保持原始 12-subset 宏平均，`±` 为三个 seed 的样本标准差，**不是三个类型均值的简单平均**。E0 只评测一次。空单元格表示需要补实验；表格定稿后才加最优值粗体。
...
[LaTeX 表体](iclr2027/tables/bright_main_k0.tex)。主表每个区块仅含 E0、InfoNCE、LambdaLoss 和拟定的 `K=0 + pairwise` RELER；reward 与 RL 组件变体分别放正文表 2、表 4。已测 `K=7` 的 0.6B 23.04 见附录表 A，4B 29.00 以及经 BRIGHT 校准的 `K=7, LR=2e-4` 29.84 ± 0.07 见[4B 附录配方表](iclr2027/tables/recipes_4b.tex)，均不填入主表的 `K=0` 行。4B `K=0` 沿用 LoRA rank 16 和其余训练协议，但按本次决定使用 LR `2e-4`，相对旧 `K=7` 配方同时改变了 `K` 和学习率；目前尚未训练。GPT-reasoning 的分数目前均未进入本地汇总，故对应两个区块保持空白。
...
[LaTeX 表体](iclr2027/tables/candidate_count_graded.tex)。`K=0` 相对 `K=7` 的逐 seed 提升为 +0.33/+0.16/+0.24；`K=0` 与 `K=1` 的平均差仅 +0.04。这张紧凑表直接解释为何改变默认候选构造；原来 `K=15` 下的 `T`、hard fraction 和候选来源搜索仍放附录。
...
[LaTeX 表体](iclr2027/tables/pairwise_k_sweep.tex)。`K=0` 行共用主表结果，`K=1` 三 seed 尚待填；`K=7/15` 是已有匹配 reward 的结果。原 `K=15, λ=0.25` 的探索结果 22.51 ± 0.23 可在附录文字中报告，不放进固定 reward 的 `K` 表。
...
[LaTeX 表体](iclr2027/tables/alignment_k0_pairwise.tex)。`ρ=0.70` 行与 0.6B 主配方共用三 seed；其余三点各需三 seed。旧 [`alignment.tex`](iclr2027/tables/alignment.tex) 虽覆盖更宽的 `ρ=0.40–0.98`，但使用纯 graded、device-local 候选，不能回答新 `K=0 + pairwise` 配方的敏感性。本表用于检查 `ρ=0.70` 的稳健性；若结果促使主配方改用其他 `ρ`，正文 reward/RL 消融的比较基座和表注也要按最终配方重审，不能混用旧 `ρ` 的结果。
...
[LaTeX 表体](iclr2027/tables/pairwise_lambda_k0.tex)。`λ=0/0.5` 分别复用正文 reward 表的现有纯 graded 三 seed 和待完成主配方三 seed；只为 `λ=1.0` 新增三 seed。旧 `K=15, λ=0.25` 不填入此表。此表与附录表 A 分开，避免混合 `K` 与 reward 权重两个变量。

## 其余现有表格的定稿位置

| 论文位置 | 表格 | 定稿处理 |
|---|---|---|
| 正文表 4 | [RL 组件消融草稿](iclr2027/tables/rl_components_k0.tex) | 固定 `K=0` 纯 graded reward；复用已有 CMP product 三 seed，核对路径等价后补 RLOO product/paired/单侧 policy 四变体 × 三 seed；旧机制分数不填入 |
