...
|---|---|---|---|
| 正文，优先 | **F1：匹配条件下的梯度方差** | 两面板：A，**单 draw 梯度云**——一个代表 probe 的 64 次 rollout，完整参数梯度在固定随机 ±1 轴上的坐标（横轴取信号分量最大的轴、纵轴取最小者，按投影信号归一；灰线连接同一 draw 的两个估计器），直接呈现"中心重合、半径小 ~20×"；CMP 团簇在云的尺度下接近一个点，右上插图按其自身尺度放大并标注两估计器均值（RLOO 均值落在团簇内）；B，全部 9 个 probe 的单步信噪比（`√signal²/noise_rms`，"signal = noise" 参考线）。方差比与 `V×t` 保留在 `probes.csv` 与图注（等效说法：RLOO 需平均 ~300–600 个独立 rollout 梯度才追上 CMP 单步），不单独成面板。以 probe 为点，不把 rollout draws 当独立训练 seed。 | 已有 local-pool、单 reward 的探针可作附录历史对照；新主配方 `K=0 + pairwise` 的完整估计器对照已于 2026-09-25 运行完毕（三个固定状态 × 3 microbatch × 64 draws，结果见 [`gradient_probe_k0_pairwise`](../analysis_results/gradient_probe_k0_pairwise/README.md)），V_CMP/V_RLOO ≈ 0.002–0.003；同日完成逐 draw 投影补采并按此设计出图。 |
| 正文，优先 | **F2：训练 reward 动态** | 两面板：A，`K=0` 的 RELER 与纯 graded 两组三 seed 的**同定义** graded nDCG，横轴 optimizer step；B，仅 RELER 的 graded 项、`0.5×pairwise` 项与 combined reward。三 seed 均值加样本 SD，附录留单 seed 原线。 | 六条 `K=0` history 已于 2026-09-24 导出至 [`reward_curves_k0`](../analysis_results/reward_curves_k0/README.md)（主配方三 seed 当日完成，最终 BRIGHT Avg. 23.38 ± 0.17）；`scripts/export_reward_curves_k0.py` 可复算。 |
...
实现状态（2026-09-25）：扩展已完成。`--compare-gradient-estimators` 遇 `reward_shortlist_count>0` 自动走 shortlist 配对模式（`probe_paired_shortlist_gradients`），逐 draw 捕获并 hash 校验动作、shortlist 身份、graded reward 表与 pairwise 输入；RLOO 对照为脚本内 `rloo_pairwise_shortlist_loss`：graded 项沿用训练 score-function 分支，pairwise 项与 CP 版同 pair 集、同 0/0.5/1 reward、同逐轴 LOO 与归一化，仅把梯度载体换成 vMF log-prob（document 侧端点局部，对应 CP 的端点局部性；各候选独立采样保证无偏）。`--self-check` 合成张量校验通过：reward 逐对一致、两估计器均值梯度在 5×MC 误差内相容、RLOO 均值梯度与有限差分参考一致（CP 对 FD 的残差仅报告不门限——两估计器同坐标同向偏离而彼此一致，指向参考噪声而非估计器）。分项梯度经 objection pass 记录（同 seed 重放，pairwise 置零 / graded reward 置零，记范数、夹角与 `g_total−(g_graded+λ·g_pairwise)` 分解残差），`--no-component-passes` 可跳过。E0 状态 2-draw CPU smoke 全部校验通过（配对 hash 一致、pairwise reward 差恰为 0、均值梯度差在 MC 误差内）。运行入口为本机脚本 `scripts/f1_probe_local.sh`（不入库；显存+利用率双低才认空闲 GPU，支持 `--state` 单状态并行）。**2026-09-25 已完成正式测量**：E0/step50/step113 三状态各 3 microbatch（位置 0/100/200）× 64 draws、含分项 pass，本机三卡并行（每进程峰值 ~33 GB）；结果与图由 `scripts/analyze_f1_probe.py` 汇总至 `paper/analysis_results/gradient_probe_k0_pairwise/`——`V_CMP/V_RLOO` 几何均值 0.0019/0.0029/0.0031（降噪 300–600 倍），`V×t` 比值仍低两个半数量级；9 probe 中 7 个均值梯度差未分辨、其余偏差 ≤ signal² 的 3%；pairwise reward 逐对差为 0；分项夹角从 E0 的 ~0.6 收缩到 step 113 的 ≈0，分解残差 ≤1.4%。同日完成逐 draw 随机投影补采（`--state proj-*`，三状态并行；8 条固定 ±1 轴、种子 20260925，与正式 probe 同种子同 microbatch，汇总脚本逐 draw 比对 hash 一致后才绘图），主图改为"梯度云 + 单步信噪比"双面板；坐标为与范数 √d 符号向量的点积，单轴方差期望即全空间 `noise_variance`，最大实测/期望比 1.19×/1.28×（n=64 的 χ² 波动内），云的半径比 20–23× 与 √方差比吻合。
...
F2 已于 2026-09-24 导出（`paper/analysis_results/reward_curves_k0/`）。F1 已于 2026-09-25 完成测量与汇总（`paper/analysis_results/gradient_probe_k0_pairwise/`）。F3/F5 在最终 checkpoint 和表格分数锁定后运行。附录 F4 可随时从已有 JSON 重绘，但必须采用订正后的统计解释。
