...
def load_official_bright(task, subsets: list[str], args: EvalArguments) -> None:
    ...
    if args.bright_query_set == "gpt-reasoning":
        ...
        for subset, subset_queries in reasoning_queries.items():
            ...
            reasoning_ids = set(subset_queries)
            if reasoning_ids != original_ids:
                raise RuntimeError(
                    f"BRIGHT {subset!r} query IDs changed while loading GPT reasoning: "
                    f"missing={sorted(original_ids - reasoning_ids)[:5]}, "
                    f"unexpected={sorted(reasoning_ids - original_ids)[:5]}"
                )
            queries[subset][BRIGHT_SPLIT] = subset_queries
    task.corpus = corpus
