...
def main() -> None:
    ...
    if cli_args and pathlib.Path(cli_args[0]).suffix.lower() in {".json", ".yaml", ".yml"}:
        ...
        model_args, data_args, training_args, lora_args, baseline_args = parse_config_file(
            parser=parser,
        ...
        config_path = None
        model_args, data_args, training_args, lora_args, baseline_args = parse_config_from_base_overrides(
            parser=parser,
            base_overrides=base_overrides,
            cli_args=cli_args,
        ...
        config_path = None
        model_args, data_args, training_args, lora_args, baseline_args = parser.parse_args_into_dataclasses(cli_args)
    ...
    logger.info("Baseline parameters %s", baseline_args)
    ...
    if trainer.is_world_process_zero():
        ...
        torch.save(baseline_args, os.path.join(training_args.output_dir, "baseline_args.bin"))
    print("Training done.")
