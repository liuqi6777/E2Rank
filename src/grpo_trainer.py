...
def restore_exploration_state(model, checkpoint_dir):
    ...
    if payload.get("reward_cross_device_negatives", False) != getattr(head, "reward_cross_device_negatives", False):
        raise ValueError("Checkpoint reward candidate pool differs; start a new run")
    if payload.get("cross_query_document_gradients", False) != getattr(head, "cross_query_document_gradients", False):
...
class EmbeddingTrainerMixin:
    ...
    def _save(self, output_dir=None, state_dict=None) -> str:
        ...
        if self.is_world_process_zero() and head is not None and hasattr(head, "exploration"):
            ...
            if getattr(head, "reward_cross_device_negatives", False):
