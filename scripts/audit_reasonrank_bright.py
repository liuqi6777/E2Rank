"""Audit pinned ReasonRank RL files against BRIGHT queries (no model calls).

Run with: uv run --no-project --with pyarrow --with scikit-learn --with rapidfuzz \
  python scripts/audit_reasonrank_bright.py
Inputs and download fingerprints: data/audit_reasonrank_bright/.
"""
from pathlib import Path
import sys, re, json, math, hashlib, unicodedata
from collections import Counter, defaultdict
import numpy as np
import pyarrow.parquet as pq
from sklearn.feature_extraction.text import TfidfVectorizer
from rapidfuzz.fuzz import ratio, token_set_ratio
from convert_reasonrank import parse_query_and_passages, parse_teacher_ranking, _user_prompt

ROOT=Path(__file__).resolve().parents[1]
BASE=ROOT/'data/audit_reasonrank_bright'
OUT=BASE/'results';OUT.mkdir(exist_ok=True)
def norm(s):
 return ' '.join(unicodedata.normalize('NFKC',s).casefold().split())
def loose(s): return ' '.join(re.findall(r'\w+',norm(s)))
def stats(a):
 return {k:round(float(v),3) for k,v in zip(['min','p25','median','p75','p95','max'],np.percentile(a,[0,25,50,75,95,100]))} if a else {}
def ndcg(final, relevant):
 rel=set(relevant);ideal=sum(1/math.log2(i+2) for i in range(min(10,len(rel))))
 return sum((d in rel)/math.log2(i+2) for i,d in enumerate(final[:10]))/ideal if ideal else 0
rr=[];errors=[]
