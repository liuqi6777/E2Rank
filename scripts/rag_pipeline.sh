...
case "$command_name" in
  prepare)
    PYTHONPATH=src python -m rag.prepare "$@"
    ;;
  encode)
    PYTHONPATH=src torchrun --standalone --nproc_per_node=4 -m rag.encode_corpus "$@"
    ;;
  candidates)
  ...
  train)
    ...
    shift
    PYTHONPATH=src torchrun --standalone --nproc_per_node=4 src/train_rag.py "$config" "$@"
    ;;
  eval)
    PYTHONPATH=src python src/eval_rag.py "$@"
    ;;
  tune-eval)
