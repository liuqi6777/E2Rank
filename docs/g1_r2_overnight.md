...
数据路径可迁移，文件内容须匹配其所在目录 `manifest.json` 中 `role=train` 的 SHA256；不再要求与开发机器的文件哈希相同。manifest 需记录无 dev、预处理正例选择 seed 42，并标识 `train.ready.jsonl`。其中保存的原机器绝对路径只用于识别文件名，校验读取的是 `G1.data` 指定目录中的实际文件。复用已有数据及其 manifest，不重新划分或生成训练数据。
同一个 seed 的 SF/CP 运行配置只允许在估计器、名称、输出目录上不同。

### 数据路径与 hash mismatch

新脚本默认读取 `configs/experiments_r2.yaml`，旧脚本读取 `configs/experiments.yaml`。如果训练机器以前修改过数据位置，将旧配置的 `G1.data` 填入新配置；它是目录，支持绝对路径，相对路径以项目根目录为基准。

`check` 和正式运行都会打印选用的配置、解析后的训练文件、manifest 路径和实际 SHA256。若仍报 `prepared training data hash mismatch`，错误会同时列出 manifest 中的预期哈希与文件实际哈希，此时需要核对选中的数据与 manifest 是否为配套文件。仅改变绝对/相对路径写法不会改变同一文件的哈希。
...
汇总读取各 run 保存的合同，列出其实际训练数据 SHA256。完成记录必须与合同匹配；不同数据的 SF/CP 不计算配对差，出现多个训练数据哈希时不计算总表均值/SD，`summary` 返回非零。三台机器使用的数据仍须一致。
