...
日期：2026-09-14。状态：实验设计；逐文档反事实 baseline 尚未实现，新的 reward 对比诊断尚未执行。
...
本计划补充 G1 的机制验证，不改变既定 G2 配置。本文只记录设计；目前没有新增训练配置、实现反事实 baseline 或启动实验。结果成熟后再决定如何组织论文中的机制证据与方法贡献。
