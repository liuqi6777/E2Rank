...
本轮只改变 G1-R2 alignment 0.80、CP、graded nDCG@10 配方的候选池：每条 query 可以使用八张卡当前
...
默认依次运行 **CP、alignment 0.80、seeds 42 / 3407 / 2026**，本批不包含 SF。
配置以已有 `align080_cp` 的 G64 对照为基准。其余为原固定 Qwen revision、
...
nohup python -u scripts/run_g1_rl_large_pool_r2.py train > g1_rl_large_pool_cp_align080.log 2>&1 &
tail -f g1_rl_large_pool_cp_align080.log
```
...
checkpoints/iclr2027-r2/
  .cl_strong_configs/G1-R2-RL-GradedNDCG64-CP-Align080-LargePool-s42.json
  G1-R2-RL-GradedNDCG64-CP-Align080-LargePool-s42/
    checkpoint-25/ ...
    exploration_state.json
    mteb_eval/bright/
  G1-R2-RL-GradedNDCG64-CP-Align080-LargePool-Seed3407-s3407/
  G1-R2-RL-GradedNDCG64-CP-Align080-LargePool-Seed2026-s2026/
```

展开配置复用启动器现有的 `.cl_strong_configs/` 目录，由独立 run ID 区分，不覆盖 CL 配置。
其他 seed 具有相同的模型/checkpoint/评测目录结构。输出名包含 `Align080`，与此前 0.90 配置区分。
